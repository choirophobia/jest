...
Expected result: **28 suites / 248 tests, all passing**, run live against the real API (no internet access = failures, since there's nothing to mock). `npm run test:smoke` runs a 9-test subset in a couple of seconds — see [Understanding Smoke Test Tagging](#understanding-smoke-test-tagging).
...
- **Create:** `POST /recipes/add` — echoes name/ingredients/difficulty/userId, checks `id` type, returns `201` like every other resource's create endpoint (this used to return `200` — see [Important Notes & Gotchas](#important-notes--gotchas) for the live API change that fixed it)
...
- **Not a resource — a payload shape every other Update test in this suite avoids on purpose.** See [Understanding the ID-in-Body Bug](#understanding-the-id-in-body-bug) below for the behavior this found, and how it changed underneath this suite mid-project.
- **Every resource checked** (`products`, `carts`, `recipes`, `comments`, `todos`, `users`, `posts`) **lets the body's `id`, when present, override the URL's id:** a matching body `id` succeeds normally; a *different* real `id` (`id: 2`) returns that record's data, not the URL's; a nonexistent body `id` (`999999`) returns `404` naming that id, even though the URL's id is perfectly valid
- **The idiomatic call still works everywhere:** omitting `id` from the body, or sending it as a matching string, both succeed exactly as every other Update test in this suite already relies on
...
**The realistic client pattern that found this.** Every Update test elsewhere in this suite sends a payload built from scratch, without an `id` field — the idiomatic way to call `PUT /{resource}/{id}`, since the id is already in the URL. But a very common real-world pattern is "fetch the resource, mutate one field client-side, `PUT` the whole object back" — React forms, admin panels, and ORM-style client SDKs all do this constantly. That payload naturally still carries the `id` field the `GET` response came with, because nobody strips it out before resubmitting. Testing *that* path — not the idiomatic one — is what surfaced everything below.

**The current, verified behavior: the body's `id`, when present, overrides the URL's id, on every resource.** `PUT /users/1` with `{ id: 2, firstName: '...' }` in the body returns **user 2's** data, not user 1's, with no indication the URL was effectively ignored — and this is uniform across `products`, `carts`, `recipes`, `comments`, `todos`, `users`, and `posts` alike. A body `id` that doesn't correspond to any real record 404s using *that* id, even though the URL's id is perfectly valid. In a real system where the URL id is what an authorization check is keyed on ("can this caller modify user `:id`?"), this pattern is the shape of an IDOR-style bug: the security check validates one id while the actual operation silently uses another.

**A real, live correction worth being transparent about.** This file originally documented a *different*, messier finding: five of these seven resources used to 404 on a body `id` that merely *equalled* the URL's id — inferred at the time to be a strict string-vs-number comparison bug (`req.params.id` arrives as a string; a JSON body's `id` is naturally a number, so `1 === '1'` failed) — while `users` and `posts` already had the override behavior described above. A few days after that version shipped, this suite's own CI started failing on exactly those tests, deterministically, twice in a row, with no `429` anywhere in sight. Curling DummyJSON directly (see [Important Notes & Gotchas](#important-notes--gotchas) for this project's general stance on a live, external, third-party API) confirmed it: DummyJSON's backend had changed. The comparison bug on those five resources is gone, and every resource now behaves the way `users`/`posts` always did. **This test file was rewritten to match reality, not defended as originally written** — the whole point of testing a live API instead of a fixture is catching exactly this, and pretending the old assertions were still correct would have defeated that.

**The idiomatic call is unaffected by any of this.** Omitting `id` from the body, or sending it as a matching string, both succeed exactly as every other Update test in this suite already relies on — that was true before DummyJSON's change and remains true after it.

**Why `DELETE` is unaffected.** `DELETE /products/1` with a mismatched `id` in the request body still deletes product 1 — confirmed explicitly, both before and after the change above. `DELETE` requests don't typically carry a semantically meaningful body in the first place, and this API's `DELETE` handlers apparently never look at one; the URL id is the only thing that matters. That's the one method where the id-in-body question doesn't apply at all.

**Why this is worth having beyond DummyJSON specifically.** An endpoint accepting the same field (`id`) from two different sources (the URL and the body) needs a single, explicit rule for which one wins when they disagree — silently trusting whichever one an author didn't expect a client to send is a common, easy-to-introduce class of bug in any REST API with resource ids in both the path and the payload, and it's specifically invisible to a test suite that only ever sends the "obviously correct" request shape.
...
**What testing it found: `null` means "not provided," uniformly — but an empty string is a real value.** `PATCH /products/1` with `title: null` leaves the title exactly as it was — the field is silently skipped, not nulled out. The same is true for `price: null`, and for a nested `{ address: { city: null } }` on `users` — only the `city` key is skipped, while every sibling nested field (`postalCode`, `country`, etc.) still deep-merges correctly around it, confirming this is a value-level check on `city` specifically, not a failure of the nested merge itself (see [Schema Validation with Zod](#schema-validation-with-zod) for how thoroughly the shape of these objects is otherwise validated). `PATCH /todos/1` with `completed: null` behaves the same way too. But `title: ""` — an explicit empty string, not `null` — **does** get applied: the title actually becomes `""` in the response. `null` and `""` are both "falsy" in JavaScript, but this API treats them as meaningfully different: one means "don't touch this field," the other means "here's a real value, which happens to be empty."

**A correction worth being transparent about.** This file originally reported `todos` as an *inconsistent exception* — applying `completed: null` where `products`/`users` ignored the equivalent `null`. Re-verifying it while investigating a separate, unrelated CI failure (see [Understanding the ID-in-Body Bug](#understanding-the-id-in-body-bug) for the fuller story of DummyJSON's backend changing mid-project) found that `todos` now ignores `null` too — the inconsistency is gone, and every resource checked agrees on `null`. What replaced it is arguably the more interesting finding: the `null`-vs-`""` distinction above, which wasn't visible in the original version of this file because it only tested `title: ""` and `title: null` as if they were two examples of the same thing, rather than two deliberately different values.

**Why this is a real, practical problem and not just a curiosity.** A client trying to implement "clear this field" as `PATCH { title: null }` will believe it succeeded — the response is `200`, no error, nothing indicates the request was ineffective — while the field silently keeps its old value. That's a worse failure mode than a `400`: a `400` at least tells the client something went wrong. The `null`-vs-`""` split cuts the other way too: a client that assumes *all* falsy values are ignored (having tested only with `null`) would be surprised that `""` actually lands. Silent no-ops on falsy input are notoriously hard to catch in manual testing, precisely because the "does this normally work" happy-path check (`PATCH` with a real, non-empty value) always passes — the bug (or, here, the nuance) only shows up for the one class of input nobody thinks to try until they specifically need to send it.
...
- **Live API drift, observed directly, more than once.** `POST /recipes/add` used to return `200` while every other resource's create endpoint returned `201` — this suite documented the inconsistency and tested for it. DummyJSON has since standardized it to `201` like everything else, which broke that test deterministically (not a `429`) until it was updated to match. Two unrelated bugs this project had documented and tested for (see [Understanding the ID-in-Body Bug](#understanding-the-id-in-body-bug) and [Understanding Falsy Value Handling on Updates](#understanding-falsy-value-handling-on-updates)) were also fixed server-side around the same time. Don't assume a status code or behavior documented here is permanent — DummyJSON is a live, external, evolving API, and this project's own history is proof it changes. Check the real response before trusting anything written down, including this file.
...
**Example from this project:** this entire suite is API-only — there's no browser involved anywhere. `tests/products.test.js` asserts directly on `res.status` and `res.data`, not on anything rendered. That's *why* it can run all 248 tests in under a minute against a live external service — a UI suite covering the same ground would take dramatically longer and be far more prone to unrelated failures.
...
**Example from this project:** `npm run test:smoke` runs 9 tests (one core read per resource, plus login) in about 2 seconds, versus the full suite's ~248 tests in roughly a minute. Critically, the smoke subset **tags existing tests** rather than duplicating them into a separate file — seeing why that distinction matters (and not just "add more tests") is itself a good interview signal; see [Understanding Smoke Test Tagging](#understanding-smoke-test-tagging).
